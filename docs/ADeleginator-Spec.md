...
| **AdminSDHolder** | A special Active Directory container object whose ACL is used as a template. The Security Descriptor Propagator (SDProp) process periodically stamps (copies) AdminSDHolder's ACL onto AdminSDHolder-protected (SDProp in-scope) principals — i.e., members of well-known protected groups identified by their SIDs. Insecure ACEs on AdminSDHolder therefore propagate to all such principals. |
