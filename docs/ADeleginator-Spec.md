...
| **AdminSDHolder** | A special Active Directory container object whose ACL is used as a template. The Security Descriptor Propagator (SDProp) process periodically stamps (copies) AdminSDHolder's ACL onto AdminSDHolder-protected (SDProp in-scope) principals — i.e., security principals associated with well-known protected SIDs (including well-known groups and certain well-known accounts, for example `{domainSID}-500`, `{domainSID}-502`). Insecure ACEs on AdminSDHolder therefore propagate to all such principals. |
