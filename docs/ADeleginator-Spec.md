...
| **AdminSDHolder** | A special Active Directory container object whose ACL is used as a template. The Security Descriptor Propagator (SDProp) process periodically stamps (copies) AdminSDHolder's ACL onto objects marked as protected (Tier 0 accounts and groups). Insecure ACEs on AdminSDHolder therefore propagate to all such objects. |
